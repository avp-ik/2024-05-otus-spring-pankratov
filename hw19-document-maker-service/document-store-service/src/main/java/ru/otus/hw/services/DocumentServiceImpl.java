...
public class DocumentServiceImpl implements DocumentService {
    ...
    @Override
    public Document get(UUID uuid) throws IOException, NoSuchAlgorithmException {
    ...
    @Override
    public Document put(Document document) throws IOException, NoSuchAlgorithmException {

        document.setId(UUID.randomUUID());
        try {
            MinioClient minioClient = prepareMinio();
            // Create some content for the object.
            {
                ...
                minioClient.putObject(
                        PutObjectArgs.builder()
                                .bucket(minioConfig.getBucketName()).object(document.getId().toString())
                                .stream(bais, bais.available(), -1).build());
                bais.close();
                System.out.println("Name=" + document.getName()
                        + ", id=" + document.getId().toString() + " is uploaded successfully");
            }
        } catch (InvalidKeyException e) {
            System.out.println("Error occurred: " + e);
        } catch (MinioException e) {
    ...
    private MinioClient prepareMinio() throws IOException, NoSuchAlgorithmException {
